...
def _upad_input(query_layer, key_layer, value_layer, o_layer,
                do_layer, dq_layer, dk_layer, dv_layer, softmax_lse,
                attention_mask, query_length, n_heads):
    ...
    dk_layer = index_first_axis(
        dk_layer.reshape(batch_size * kv_seq_len, num_key_value_heads, head_dim), indices_k  # filter out the key with unmask query
    )
    value_layer = index_first_axis(
        value_layer.reshape(batch_size * kv_seq_len, num_key_value_heads, head_dim), indices_k
    )
    dv_layer = index_first_axis(
        dv_layer.reshape(batch_size * kv_seq_len, num_key_value_heads, head_dim), indices_k
    )
...
@pytest.mark.parametrize(
    "seqlen_q,seqlen_k",
    [
        # (113, 203), # (wenting.swt): single test passed
        (8, 8),
        (128, 128),
        (2048, 2048),
    ],
