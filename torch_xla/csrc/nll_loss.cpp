...
WeightScale DynamicGetMaskedWeight(xla::XlaOp weight, xla::XlaOp logits, const xla::Shape& logits_shape,
  ...
  xla::XlaOp xvalid_bitmap = DynamicBroadcastInDim(valid_bitmap, sym_op, broadcast_dims, valid_shape);
...
xla::XlaOp BuildNllLoss(xla::XlaOp logits, xla::XlaOp labels, xla::XlaOp weight,
  ...
  if (XlaHelpers::IsStableHloEnabled() && logits_shape.is_dynamic()) {
    weight_scale = DynamicGetMaskedWeight(weight, logits, logits_shape, labels, one_hot_labels,
