...
XlaOpVector Select::Lower(LoweringContext* loctx) const {
  ...
  if (XlaHelpers::IsDISCBackend() && input_shape.is_bounded_dynamic()) {
    xla::XlaOp start =
        XlaHelpers::ScalarValue<int32_t>(start_, input.builder());
    xla::XlaOp end = XlaHelpers::ScalarValue<int32_t>(end_, input.builder());
    xla::XlaOp stride =
        XlaHelpers::ScalarValue<int32_t>(stride_, input.builder());
    xla::Shape final_shape =
        MakeSelectShape(input_shape, dim_, start_, end_, stride_);
    xla::XlaOp output =
        BuildSelectSymInt(input, dim_, start, end, stride, final_shape);
    return ReturnOp(output, loctx);
  } else if (!input_shape.is_unbounded_dynamic()) {
