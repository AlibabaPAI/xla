...
void custom_call_flash_attention_forward(cudaStream_t stream, void** buffers,
  ...
  at::Tensor out = torch::from_blob(
      buffers[4 + buf_offset], {params.b, params.seqlen_q, params.h, params.d},
      opts.dtype(scalar_type));

  const int seqlenq_ngroups_swapped =
      params.seqlen_q == 1 && params.h > params.h_k &&
      params.window_size_left < 0 && params.window_size_right < 0 &&
      1 - params.p_dropout == 0.f && params.h % 8 == 0 &&
      !params.enable_alibi_slopes;
...
XlaOpVector FlashAttentionForward::Lower(LoweringContext* loctx) const {
  ...
  std::vector<xla::XlaOp> result =
      BuildFlashAttentionForward(q, k, v, alibi_slopes, params_, xla_shape());
