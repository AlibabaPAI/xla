...
SelectSymInt::SelectSymInt(const torch::lazy::Value& input,
                           int64_t dim,
    ...
    : XlaNode(
          xla_select, {input, start, end, stride},
          output_shape,
          /*num_outputs=*/1, torch::lazy::MHash(dim)), dim_(dim) {}
...
XlaOpVector SelectSymInt::Lower(LoweringContext* loctx) const {
  ...
  //   start_ops[i] = xla::Reshape(start_ops[i], {1});
  //   limit_ops[i] = xla::Reshape(limit_ops[i], {1});
  //   stride_ops[i] = xla::Reshape(stride_ops[i], {1});
  // }

  // xla::XlaOp concat_start = xla::ConcatInDim(input.builder(), start_ops, {0});
  // xla::XlaOp concat_limit = xla::ConcatInDim(input.builder(), limit_ops, {0});
  // xla::XlaOp concat_stride = xla::ConcatInDim(input.builder(), stride_ops, {0});

  // xla::XlaOp output =
  //       xla::CustomCall(input.builder(), "mhlo.real_dynamic_slice",
  //                       /*operands=*/{input, concat_start, concat_limit, concat_stride},
  //                       /*shape*/ xla_shape());
  // return ReturnOp(output, loctx);
}
