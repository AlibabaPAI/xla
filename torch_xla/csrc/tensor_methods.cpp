...
void arange_out(XLATensorPtr& out, const at::Scalar& start,
  ...
  if (XlaHelpers::IsDISCBackend() &&
      (start.isSymbolic() || end.isSymbolic() || step.isSymbolic())) {
    XLA_CHECK(start.isIntegral(/*includeBool=*/false) &&
              end.isIntegral(/*includeBool=*/false) &&
              step.isIntegral(/*includeBool=*/false))
        << "Only int start && end && step is supported, got " << start << ", "
        << end << ", " << step;
    // corner case
    ...
    xla::PrimitiveType prim_type =
        MakeXlaPrimitiveType(scalar_type, &out->GetDevice());
    auto range_size =
        1 + (end.toSymInt() - start.toSymInt() - 1) / step.toSymInt();  // ceil
    ...
    out->SetIrValue(DynamicArange(size_value, start_value, step_value,
                                  prim_type, result_shape));
    out->SetScalarType(scalar_type);
  } else {
...
XLATensorPtr embedding_symint(const XLATensorPtr& weight,
  ...
  XLATensorPtr embeddings =
      index_select(weight, 0, view_symint(indices, {indices_numel}));
...
void copy_symint_(XLATensorPtr& input, c10::SymIntArrayRef input_size,
  ...
  if (input->GetDevice() == src->GetDevice()) {
    ...
    input->SetIrValue(
        MaybeExpandSymInt(copy_value, input->shape(), input_size));
  } else {
    auto input_shape = input->shape();
...
XLATensorPtr slice_symint(const XLATensorPtr& input, int64_t dim,
                          c10::SymInt start, c10::SymInt end,
                          c10::SymInt step) {
  auto input_shape = input->shape();
  dim = torch::lazy::GetCanonicalDimensionIndex(dim, input_shape.get().rank());
  std::vector<int64_t> input_dims = torch_xla::runtime::util::ToVector<int64_t>(
  ...
  // TODO: support this
  XLA_CHECK(start_upper >= 0 && start_upper <= input_dims[dim])
      << "Dynamic slice with start < 0 or start > size is not supported";
  XLA_CHECK(end_upper >= 0 && end_upper <= input_dims[dim])
      << "Dynamic slice with end < 0 or end > size is not supported";
  XLA_CHECK(start_upper <= end_upper)
      << "Dynamic slice with start > end is not supported";

  xla::Shape output_shape = SelectSymInt::MakeSelectShape(
      input->shape(), dim, start_upper, end_upper, GetSymIntUpperBound(step));
  ...
  return input->CreateFrom(
      torch::lazy::MakeNode<SelectSymInt>(input->GetIrValue(), dim, start_value,
                                          end_value, step_value, output_shape));
}
...
XLATensorPtr unsqueeze_symint(const XLATensorPtr& input,
  ...
  final_sizes.insert(final_sizes.begin(), input_sizes.begin(),
                     input_sizes.end());
  final_sizes.insert(final_sizes.begin() + dim, c10::SymInt(1));
  return view_symint(
      input, c10::SymIntArrayRef(final_sizes.data(), final_sizes.size()));
}
...
XLATensorPtr view_symint(const XLATensorPtr& input,
  ...
  return input->CreateFrom(torch::lazy::MakeNode<ViewSymIntOp>(
      input->GetIrValue(), size_elements, result_shape));
}
