...
genrule(
    name = "build_disc",
    outs = ["libral_base_context.so", "disc_compiler_main", "torch-mlir-opt"],
    local = True,
    cmd = ';'.join(['export PATH=/root/bin:/usr/local/cuda/bin:$${PATH}',
                    'pushd third_party/BladeDISC/pytorch_blade',
                    'python ../scripts/python/common_setup.py',
                    'TF_CUDA_COMPUTE_CAPABILITIES="7.0,8.0,8.6,9.0" TORCH_CUDA_ARCH_LIST="7.0 8.0 8.6 9.0" python setup.py bdist_wheel 2>&1 | tee build.log',
                    'popd',
